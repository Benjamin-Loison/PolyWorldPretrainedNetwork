...
def prediction(batch_size, images_directory, annotations_path):
    ...
    model.load_state_dict(torch.load("./trained_weights/polyworld_backbone").to(device))
    head_ver.load_state_dict(torch.load("./trained_weights/polyworld_seg_head").to(device))
    matching.load_state_dict(torch.load("./trained_weights/polyworld_matching").to(device))
